...
<%@ page import="com.cya.pojo.Notice,com.cya.pojo.Type,com.cya.dao.AdminDao,com.cya.dao.TypeDao,com.cya.dao.BookDao,com.cya.pojo.Admin" %>
...
<html>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <table id="data_list" class="table table-hover table-bordered" cellspacing="0" width="100%">
                        ...
                        <!---在此插入信息-->
                        <%
                            ArrayList<Notice> noticeData = new ArrayList<Notice>();
                            NoticeDao noticedao = new NoticeDao();
                            noticeData = (ArrayList<Notice>) noticedao.get_ListInfo();
                            

                            for (Notice bean : noticeData) {
                        %>
                        <tbody>

                        <td><%= bean.getnId() %></td>
                        <td><%= bean.getnTime() %></td>
                        <td><%= bean.getnPerson() %></td>
                        <td><%= bean.getnContext() %></td>

                        <td>
                            <input type="hidden" id="<%= bean.getnId()%>" value="<%= bean.getnContext()%>">
                            <button type="button" class="btn btn-warning btn-xs" data-toggle="modal"
                                    data-target="#updateModal"
                                    id="btn_update" onclick="showInfo2(<%= bean.getnId() %>)">
                                                                                         修改
                            </button>
                            <button type="button" class="btn btn-danger btn-xs"
                                   onclick="deleteNotice(<%= bean.getnId() %>)">删除
                           </button>
                        </td>
                        </tbody>
                        <%} %>
    ...
    <script type="text/javascript">
        function showInfo2(nid) {
            document.getElementById("updatetid").value = nid;
            document.getElementById("updatename").value = document.getElementById(nid).value;
        }
